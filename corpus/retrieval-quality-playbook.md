...
The platform uses hybrid retrieval because pure keyword search misses loosely related passages and single-signal ranking can miss useful chunk overlap. Query terms first score every chunk with a BM25-style sparse ranker, then the same query is projected into a deterministic hashed vector sketch so adjacent lexical structure can still influence recall without introducing an external embedding dependency.
